...
# Instantiate LLM (Try Bedrock, fallback to OpenAI if no local AWS creds, else Mock)
try:
    if not boto3.Session().get_credentials():
        raise Exception("No AWS credentials found in environment.")
    llm = ChatBedrock(
    ...
    if os.environ.get("OPENAI_API_KEY"):
        logger.info("Using OpenAI fallback due to missing AWS credentials.")
        llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
        USE_MOCK = False
    else:
        logger.warning("No AWS or OpenAI creds found. Using Mock.")
        USE_MOCK = True
